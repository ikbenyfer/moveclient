...
| **Xray** | World | Two views at once: a HUD text readout of the nearest valuable ores (diamond, emerald, ancient debris, optionally gold/common ores) with direction and distance, *and* a real see-through-block resource pack — bundled in the mod jar, auto-applied/removed when the module toggles — whose covered blocks render as nothing at all (a small Mixin also fixes vanilla face-culling so fully-buried ore veins are visible too, not just already-exposed ones). | Scan Radius, Max Listed, Scan Interval, Include Gold, Include Common Ores, Texture Pack |
