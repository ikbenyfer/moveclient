...
| **Xray** | World | Periodically scans a radius around you for valuable ores (diamond, emerald, ancient debris, optionally gold/common ores) and lists the nearest ones on the HUD with direction and distance, through walls. Always shows a status line while enabled, even with zero matches. Implemented as a text readout rather than a rendered see-through overlay — see the note in `XrayModule`'s class doc for why. | Scan Radius, Max Listed, Scan Interval, Include Gold, Include Common Ores |
