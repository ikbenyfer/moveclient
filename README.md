...
| **Xray** | World | Two views at once: a HUD text readout of the nearest valuable ores (diamond, emerald, ancient debris, optionally gold/common ores) with direction and distance, *and* a real see-through-block resource pack — bundled in the mod jar, auto-applied/removed when the module toggles — that shrinks stone/dirt-family blocks to thin, fully-lit shells so ore veins show through as bright outlines. | Scan Radius, Max Listed, Scan Interval, Include Gold, Include Common Ores, Texture Pack |
