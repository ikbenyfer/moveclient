...
| **Scaffold** | Movement | Auto-places a block under your feet as you walk over air, reusing AirPlace's placement technique. Your hand always takes priority; otherwise it searches the hotbar and briefly switches to a block slot (a real, server-synced switch, so there's a one-tick flicker). | — |
...
| **AirPlace** | World | Places blocks even when nothing is in range to click against, by sending a synthetic `BlockHitResult` targeting empty air — vanilla places directly there when the clicked position is replaceable. Only activates when vanilla's own crosshair target isn't already a real block. | Range |
